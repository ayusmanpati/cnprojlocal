/*
 * This module handles all incoming HTTP requests and routes them to the appropriate handler (e.g., file handler or API handler).
 */
...
function requestHandler(req, res) {
  console.log(`Incoming request: ${req.method} ${req.url}`); // API Routes for authentication
  ...
  if (req.method === "GET") {
    ...
    switch (route) {
      case "/":
      case "/login": // Serve the main login/signup page
        return serveStaticFile(req, res, "/index.html");
      case "/chat": // Serve the chat page
        return serveStaticFile(req, res, "/chat.html");
      case "/profile": // Serve the profile page
        return serveStaticFile(req, res, "/profile.html");
      case "/about": // Serve the about us page
        return serveStaticFile(req, res, "/about.html");
    }
